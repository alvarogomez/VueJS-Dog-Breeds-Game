  ...
  export default {
    ...
    methods: {
      clickOnAnswer: function(event) {
        if (event.target.innerText.toLowerCase() == this.breeds[this.questionBreedIndex]) {
      ...
      setConcreteImage: async function() {
        let url = 'https://dog.ceo/api/breed/' + this.breeds[this.questionBreedIndex] + '/images/random' ;
